...
To build a classification methodology to determine whether a customer is placing a fraudulent (unjustifiably claimimg) insurance claim.

Ex: We might have seen that people are try to clame insurance for normal wear and tear, also some times creates fake witness for proof. To overcome this problem we are trying to build a model which will bw able to correctly indentify which claims are fraudulent and which claims are genuine. Whichever claims are predicted as fraudulent will not approved or will be pass for manual check (as per the insurance company rules).
...
5) Once the prediction is made for all the clusters, the predictions along with the index number are saved in a CSV file at a given location, and the location is returned to the client.
